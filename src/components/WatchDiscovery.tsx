    ...
    <section id="watch-discovery-section" className="py-10 md:py-16 bg-[#FAF6F0] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative group">
          ...
          {/* Left Arrow Button */}
          <button
            id="discovery-prev-button"
            onClick={() => scroll('left')}
            aria-label="Previous discovery stories"
            className="absolute -left-3 sm:-left-5 top-1/2 -translate-y-1/2 z-20 p-2 sm:p-3 rounded-full bg-[#FAF6F0] text-[#9E472A] border border-[#DFCBB8] shadow-lg hover:bg-[#9E472A] hover:text-white transition-all duration-200 cursor-pointer"
          >
          ...
          {/* Right Arrow Button */}
          <button
            id="discovery-next-button"
            onClick={() => scroll('right')}
            aria-label="Next discovery stories"
            className="absolute -right-3 sm:-right-5 top-1/2 -translate-y-1/2 z-20 p-2 sm:p-3 rounded-full bg-[#FAF6F0] text-[#9E472A] border border-[#DFCBB8] shadow-lg hover:bg-[#9E472A] hover:text-white transition-all duration-200 cursor-pointer"
          >
            <ChevronRight className="w-5 h-5" />
          </button>

          {/* Horizontal Story Grid / Scroll Container */}
          <div
            ref={scrollContainerRef}
            className="flex items-center gap-4 sm:gap-5 overflow-x-auto no-scrollbar py-2 scroll-smooth"
          ...
          >
            {stories.map((story) => (
              <div
                ...
                className="flex-none w-44 sm:w-52 md:w-60 group/item cursor-pointer transition-transform duration-300 hover:-translate-y-2"
              >
                {/* Instagram Reel Size 9:16 Thumbnail Tile */}
                <div className="relative aspect-[9/16] rounded-xl overflow-hidden bg-[#2C2420] shadow-lg group-hover/item:shadow-2xl border border-[#DFCBB8]/40">
                  ...
                  <div className="absolute top-3 inset-x-3 flex items-center justify-between z-10">
                    <span className="px-2 py-0.5 rounded-full bg-black/60 backdrop-blur-md text-[10px] font-cinzel font-semibold tracking-wider text-white border border-white/20 uppercase">
                      REEL
                    </span>
